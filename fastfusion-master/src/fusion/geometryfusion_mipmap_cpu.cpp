...
void FusionMipMapCPU::copyVboIndex(size_t c_elems, int *ai) {
    if (getFaceVerticesCount() != c_elems) {
        fprintf(stderr,"\n+++ %s(): fatal-0: face vertex count does't agree.  bye!\n", __FUNCTION__);
        exit(1);
    }
    std::vector<unsigned int> *p_faces = &_meshCurrent->faces;
    std::vector<unsigned int> &faces = *p_faces;
    int verticesPerFace = _meshCurrent->_verticesPerFace;
    if (verticesPerFace != 3) {
        fprintf(stderr,"\n+++ %s(): fatal-1: only triangle mesh elements supported.  bye!\n", __FUNCTION__);
        exit(1);
    }
    for (unsigned int i = 0; i < c_elems; i++) {
        ai[i] = faces[i];
    }
}
void FusionMipMapCPU::copyVboVertex(size_t c_elems, float *af_scaled,
                                    float f_scale) {
    size_t c_vertex = getVertexCount();
    if (c_vertex != c_elems/6) {
        fprintf(stderr,"\n+++ %s(): fatal: vertex counts don't agree.  bye!\n", __FUNCTION__);
        exit(1);
    }
    std::vector<Vertex3f> *p_vertices = &_meshCurrent->vertices;
    std::vector<Vertex3f> &vertices = *p_vertices;
    std::vector<Color3b> *p_colors = &_meshCurrent->colors;
    std::vector<Color3b> &colors = *p_colors;

    bool has_color = colors.size() ? true : false;
    for (unsigned int i = 0; i < c_vertex; i++) {
        af_scaled[i*6] = vertices[i].x * f_scale;
        af_scaled[i*6+1] = vertices[i].y * f_scale;
        af_scaled[i*6+2] = vertices[i].z * f_scale;
        af_scaled[i*6+3] = has_color ? (colors[i].r)/255. : 0.;
        af_scaled[i*6+4] = has_color ? (colors[i].g)/255. : 0.;
        af_scaled[i*6+5] = has_color ? (colors[i].b)/255. : 0.;
    }
}

void FusionMipMapCPU::copyPlyVertex(size_t c_vertex,
    float *af_0, float *af_1, float *af_2, int *ai_0, int *ai_1, int *ai_2) {
    if (getVertexCount() != c_vertex) {
        fprintf(stderr,"\n+++ %s(): fatal: vertex counts don't agree.  bye!\n", __FUNCTION__);
        exit(1);
    }
    std::vector<Vertex3f> *p_vertices = &_meshCurrent->vertices;
    ...
    for (unsigned int i = 0; i < c_vertex; i++) {
        ...
        af_2[i] = vertices[i].z;
        if (has_color) {
            ai_0[i] = (int)(colors[i].r);
            ai_1[i] = (int)(colors[i].g);
            ai_2[i] = (int)(colors[i].b);
        }
    }
}
void FusionMipMapCPU::copyPlyFace(size_t c_faces,
    int *ai_0, int *ai_1, int *ai_2, int *ai_3) {
    if (c_faces != getFaceVerticesCount()/3) {
        fprintf(stderr,"\n+++ %s(): fatal-0: face counts don't agree.  bye!\n", __FUNCTION__);
        exit(1);
    }
    std::vector<unsigned int> *p_faces = &_meshCurrent->faces;
    std::vector<unsigned int> &faces = *p_faces;
    ...
    for (unsigned int i = 0; i < c_faces; i++) {
        ai_0[i] = 3;
        ai_1[i] = faces[3*i];
        ai_2[i] = faces[3*i+1];
        ai_3[i] = faces[3*i+2];
...
void FusionMipMapCPU::writePLY(const std::string& filename) {
    fprintf(stderr,"\n+++ %s(): writing %s.ply\n", __FUNCTION__, filename.c_str());
    _meshCurrent->writePLY(filename, false);
}
void FusionMipMapCPU::printInfo() {
    fprintf(stderr,"\n+++ %s(): Interleaved Mesh-Cell Mesh has %li vertices and %li face vertices (in _meshCurrent)\n",
        __FUNCTION__, _meshCurrent->vertices.size(), _meshCurrent->faces.size());
}
size_t FusionMipMapCPU::getVertexCount() {
    return _meshCurrent->vertices.size();
}
size_t FusionMipMapCPU::getFaceVerticesCount() {
    return _meshCurrent->faces.size();
}
...
#ifdef CRAPPY_LAPPY
	fprintf(stderr, "M");
	printf(stderr, "+++ %s(): calling updateMeshes()...\n", __FUNCTION__);
	updateMeshes();
	fprintf(stderr,"!");
#endif
...
void FusionMipMapCPU::allocateSensorData(SensorData **ppSensorData) {
    assert(ppSensorData != NULL);
    assert(*ppSensorData == NULL);

    SensorData *pSensorData = (SensorData *)malloc(sizeof(SensorData));
    pSensorData->cPoints = 0;
    size_t max = 640*480;
    pSensorData->cPointsMax = max;
    pSensorData->pX = new std::vector<int>(max);
    pSensorData->pY = new std::vector<int>(max);
    pSensorData->pVertex = new std::vector<Vertex3f>(max);
    pSensorData->pRgb = new std::vector<Color3b>(max);
    *ppSensorData = pSensorData;
}
void FusionMipMapCPU::freeSensorData(SensorData **ppSensorData) {
    assert(ppSensorData != NULL);
    assert(*ppSensorData != NULL);

    SensorData *pSensorData = *ppSensorData;

    //http://stackoverflow.com/questions/10464992/c-delete-vector-objects-free-memory
    std::vector<int>().swap(*pSensorData->pX);
    std::vector<int>().swap(*pSensorData->pY);
    std::vector<Vertex3f>().swap(*pSensorData->pVertex);
    std::vector<Color3b>().swap(*pSensorData->pRgb);
    delete pSensorData->pX;
    delete pSensorData->pY;
    delete pSensorData->pVertex;
    delete pSensorData->pRgb;

    free(pSensorData);
    *ppSensorData = NULL;
}

void FusionMipMapCPU::getSensorData(SensorData *pSensorData,
    ...
    )
{
    ...
    fprintf(stderr, "p.t: %f %f %f\n", p.t1, p.t2, p.t3);

    const uchar *rgbData = (const uchar*)rgb.data;
    const int channels = rgb.channels();
    const ushort *depthData = (ushort*)depth.data;
//fixme: why error on linux?
//    const float *hData = (hdataOpt != boost::none) ? (float*)hdataOpt.get().data : NULL;
    const float *hData = hdataOpt ? (float*)hdataOpt.get().data : NULL;
    ...
    if (nxny > pSensorData->cPointsMax) {
        fprintf(stderr, "%s(): resizing to %ld\n", __FUNCTION__, nxny);
        rSensorX.resize(nxny);
        rSensorY.resize(nxny);
        rSensorVer.resize(nxny);
        rSensorRgb.resize(nxny);
        pSensorData->cPointsMax = nxny;
    }

    fprintf(stderr,"+++ FusionMipMapCPU::%s(): (nx, ny): (%d, %d)\n",
            __FUNCTION__, nx, ny);
    size_t iPoint = 0;
    //fprintf(stdout, "maxcamdistance: %f\n", maxcamdistance);
    for (int y = 0; y < ny; y++) {
        for (int x = 0; x < nx; x++) {
            size_t index = y*nx+x;
            size_t indexColor = channels * index;
            float h = hData ? hData[index] : depthData[index]*scaling;
            if (h > 0.0f && h < maxcamdistance - 0.1) {
                float px, py, pz;
                float vxWorld, vyWorld, vzWorld;
                uchar b, g, r;

                px = (x-p.cx)/p.fx*h;
                py = (y-p.cy)/p.fy*h;
                pz = h;
                vxWorld = p.r11*px + p.r12*py + p.r13*pz + p.t1;
                vyWorld = p.r21*px + p.r22*py + p.r23*pz + p.t2;
                vzWorld = p.r31*px + p.r32*py + p.r33*pz + p.t3;
                // c.f. FusionMipMapCPU::getMeshStructural(...)
                //http://stackoverflow.com/questions/8932893/accessing-certain-pixel-rgb-value-in-opencv
                b = rgbData[indexColor];
                g = rgbData[indexColor+1];
                r = rgbData[indexColor+2];
                rSensorX[iPoint] = x;
                rSensorY[iPoint] = y;
                rSensorVer[iPoint] = Vertex3f(vxWorld, vyWorld, vzWorld);
                rSensorRgb[iPoint] = Color3b(r, g, b);
                iPoint++;
            }
        }
    }
    fprintf(stderr, "valid points: %ld of %ld\n", iPoint, nxny);
    pSensorData->cPoints = iPoint;
}


int FusionMipMapCPU::addMap(const cv::Mat &depth, CameraInfo caminfo,
            const cv::Mat &rgb, float scaling, float maxcamdistance)
{
	fprintf(stderr,"+++ FusionMipMapCPU::%s(): Adding Integer Image %i",
	        __FUNCTION__, _framesAdded);
	//Parameter Helpers
	cv::Mat rot = caminfo.getRotation();
	cv::Mat trans = caminfo.getTranslation();
	cv::Mat intr = caminfo.getIntrinsic();
...
//					depthdata2[y*nx+x] = 0;
            ...
            fprintf(stderr,"+++ %s(): showing new depth img...\n", __FUNCTION__);
			cv::imshow("New",depth);
...
(
		std::list<size_t> *meshCellQueue,//$$$ indices of cells in queue that get involved in update
		std::vector<bool> *meshCellIsQueued,
		FusionMipMapCPU::MeshCellArray *meshCells,//$$$ cells data to update
		ParentArray *leafParent,
		MarchingCubesIndexed *mc,
		treeinfo *info,
		volatile int *meshingDone,
		MeshInterleaved *mesh,//$$$ output !!!!
		std::vector<FusionMipMapCPU::MeshStatistic> *meshTimes
)
{
	...
	size_t oldSize = meshCellQueue->size();
	fprintf(stderr,"\n%s(): meshCellQueue->size(): %li",__FUNCTION__,oldSize);
	size_t meshcellsSize = meshCells->size();
	double timeBefore = (double)cv::getTickCount();//-------------------------------------

	for(std::list<size_t>::iterator i=meshCellQueue->begin();i!=meshCellQueue->end();){
		if(*i>=meshCells->size()) fprintf(stderr,"\n%s(): ERROR:Wrong Index in MeshCell Queue!: %li >= %li",__FUNCTION__,*i,meshCells->size());
		(*meshCells)[*i].updateMesh(*info,*leafParent,*mc);
...
//	fprintf(stderr,"\nLast leaf for Mesh Cell %li is %i",11585,(*meshCells)[11585].lastLeaf[0]);
	...
	*mesh = MeshInterleaved(3);
	...
	fprintf(stderr,"\n%s(): Summing up %li Mesh Cells...",__FUNCTION__,meshcellsSize);
...
//	*mesh = *((*meshCells)[11585].meshinterleaved);
	...
	double timeUpdate = timeMiddle-timeBefore;
	double timeSum = timeAfter-timeMiddle;
    fprintf(stderr,"\n%s(): timeUpdate: %f msec",__FUNCTION__,1000*timeUpdate/cv::getTickFrequency());
    fprintf(stderr,"\n%s(): timeSum: %f msec",__FUNCTION__,1000*timeSum/cv::getTickFrequency());

	if(meshTimes) meshTimes->push_back(FusionMipMapCPU::MeshStatistic(0,oldSize,meshcellsSize,timeUpdate,timeSum));
	fprintf(stderr,"\n%s(): Meshes summed up.",__FUNCTION__);
	*meshingDone = 0;
}


//+++ this is primarily called from fusionWrapper() in onlinefusionviewer.cpp
bool FusionMipMapCPU::updateMeshes()
{
    fprintf(stderr,"\n+++ %s(): _meshingDone: %d\n",__FUNCTION__,_meshingDone);
	if(_meshingDone==0){
		double time5 = (double)cv::getTickCount();
		fprintf(stderr, "+++ %s(): U[Q:%li]",__FUNCTION__,_meshCellQueueNext.size());
		_meshingDone = 1;
		...
		fprintf(stderr,"\n+++ %s(): Switching MeshCellQueue",__FUNCTION__);
		...
		fprintf(stderr,"\n+++ %s(): MeshCellQueue switched",__FUNCTION__);
...
//					&_leafParentCopy,&_mc,&_treeinfo,&_meshingDone,_meshSeparateNext,&_meshTimes);
			...
			fprintf(stderr,"\n+++ %s(): !!!! Calling meshWrapperInterleaved() without Threading",__FUNCTION__);
		    ...
		    //$$$ now, _meshNext is the latest result
...
//			separate = _meshSeparateCurrent; _meshSeparateCurrent = _meshSeparateNext; _meshSeparateNext = separate;
			...
			//$$$ now, _meshCurrent is the latest result

			double diffTime;
			double time6 = (double)cv::getTickCount();
			diffTime = time6-time5;
			fprintf(stderr,"\n+++ %s(): time6-time5: %f msec",__FUNCTION__,1000*diffTime/cv::getTickFrequency());
...
MeshSeparate FusionMipMapCPU::getMeshSeparateMarchingCubes(MeshSeparate mesh)
{
	...
	fprintf(stderr,"\n%s(): Getting Nonindexed Mesh Recursively",__FUNCTION__);
...
//	return getMeshRecursiveIncremental(mesh);
	...
	fprintf(stderr,"\n%s(): Getting Indexed Interleaved Mesh Approximate",__FUNCTION__);
	fprintf(stderr,"\n%s(): Interleaved Mesh-Cell Mesh has %li vertices and %li indices (_meshCurrent)",
			__FUNCTION__,_meshCurrent->vertices.size(),_meshCurrent->faces.size());

	fprintf(stderr,"\n%s(): returning _meshCurrent",__FUNCTION__);
	return *_meshCurrent;
}
