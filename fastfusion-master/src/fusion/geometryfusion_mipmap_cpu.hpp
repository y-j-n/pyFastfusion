...
typedef struct {
    ...
    std::vector<int> *pX;
    std::vector<int> *pY;
    std::vector<Vertex3f> *pVertex;
    std::vector<Color3b> *pRgb;
} SensorData;
...
public:
    ...
    void copyVboIndex(size_t c_elems, int *ai);
    void copyVboVertex(size_t c_elems, float *af_scaled, float f_scale);
    void copyPlyVertex(size_t c_vertex,
        float *af_0, float *af_1, float *af_2, int *ai_3, int *ai_4, int *ai_5);
    void copyPlyFace(size_t c_faces,
        int *ai_0, int *ai_1, int *ai_2, int *ai_3);
    void writePLY(const std::string& filename);
	void printInfo();
    size_t getVertexCount();
    size_t getFaceVerticesCount();
    ...
    static void allocateSensorData(SensorData **ppSensorData);
    static void freeSensorData(SensorData **ppSensorData);
    static void getSensorData(SensorData *pSensorData,
        const cv::Mat &rot, const cv::Mat &trans, const cv::Mat &intr,
        const cv::Mat &depth, const cv::Mat &rgb,
        float scaling, float maxcamdistance,
//        const cv::Mat *pHdataOpt);
