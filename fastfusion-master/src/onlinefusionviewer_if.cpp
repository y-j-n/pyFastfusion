...
OnlineFusionViewerInterface::OnlineFusionViewerInterface(bool createMeshList) :QGLViewer(),
...
OnlineFusionViewerInterface::~OnlineFusionViewerInterface()
{
	_timer->stop();
	delete _timer;
	_fusionAlive = false;
	...
	fprintf(stderr,"\nEnd of OnlineFusionViewerInterface Destructor.");
...
void OnlineFusionViewerInterface::enableLighting(){_lightingInternal=true;   glColor3f(0.5,0.5,0.5); glEnable(GL_LIGHTING);}
void OnlineFusionViewerInterface::disableLighting(){_lightingInternal=false; glDisable(GL_LIGHTING);}

void OnlineFusionViewerInterface::drawGridFrame(float ox, float oy, float oz, float sx, float sy, float sz)
{
	disableLighting();
...
void OnlineFusionViewerInterface::generateBuffers(){
	glewInit();
...
void OnlineFusionViewerInterface::drawMeshPointer()
{
...
void OnlineFusionViewerInterface::drawMeshInterleaved()
{
	fprintf(stderr,"\n%s(): Drawing interleaved Mesh",__FUNCTION__);
...
void OnlineFusionViewerInterface::drawCameraFrustum
(
		CameraInfo pose,
...
void OnlineFusionViewerInterface::init()
{
...
void OnlineFusionViewerInterface::setScenePosition(CameraInfo pose)
{
	CameraInfo inv = pose;
...
void OnlineFusionViewerInterface::draw()
{
...
void OnlineFusionViewerInterface::fastDraw()
{
...
void OnlineFusionViewerInterface::updateSlotIf() {
    fprintf(stderr, "@@@ %s(): call updateSlot() and sleep forever...\n", __FUNCTION__);
    updateSlot();
    sleep(99999999);
}

void OnlineFusionViewerInterface::keyPressEvent(QKeyEvent *e)
{
...
void imageReadingAndPreprocessingWrapperIf
(
...
void OnlineFusionViewerInterface::fusionWrapperIfOneStep(
...
//#else
    fprintf(stderr,"\n+++ %s(): Adding Integer Depthmap.\n", __FUNCTION__);
    fprintf(stderr,"+++ %s(): imageDepthScale: %f\n", __FUNCTION__, imageDepthScale);
    fprintf(stderr,"+++ %s(): scaling = 1.0/imageDepthScale: %f\n", __FUNCTION__, 1.0f/imageDepthScale);
    fprintf(stderr,"+++ %s(): maxCamDistance: %f\n", __FUNCTION__, maxCamDistance);
    fusion->addMap(depthimage, (*pLast)[currentFrame], rgbimage,
                   1.0f/imageDepthScale, maxCamDistance);
    fprintf(stderr,"\n+++ %s(): Added Integer Depthmap.\n", __FUNCTION__);
//#endif

#ifndef DEBUG_NO_MESHES
    fprintf(stderr,"\n+++ %s(): calling fusion->updateMeshes()\n", __FUNCTION__);
    //*newMesh = fusion->updateMeshes();
    fusion->updateMeshes();
#endif
    fprintf(stderr,"\n+++ %s(): currentFrame done >>>>>>>> >>>>>>>> >>>>>>>>\n", __FUNCTION__);
    fusion->printInfo();
    //sleep(999999);  // 1.1gb RAM consumed at this point
    //fusion->writePLY("mesh-first__renameme");  exit(0);
}

FusionWrapperIfData * OnlineFusionViewerInterface::fusionWrapperIfInit(
    ...
    FusionWrapperIfData *pFusionWrapperIfData = (FusionWrapperIfData *)malloc(sizeof(FusionWrapperIfData));
//--------
	//fprintf(stderr,"\nStarting separate Fusion Thread on Frame %li\n",*_currentFrame+1);
	//fprintf(stderr,"\nStarting fusionWrapperIf() on Frame %li\n",*_currentFrame+1);

	pFusionWrapperIfData->fusion = par.fusion;
	pFusionWrapperIfData->imageDepthScale = par.imageDepthScale;
    //printf("[ok] pFusionWrapperIfData->imaeDepthScale: %f\n", pFusionWrapperIfData->imageDepthScale);
	pFusionWrapperIfData->maxCamDistance = par.maxCamDistance;
pFusionWrapperIfData->threadImageReading = par.threadImageReading;
pFusionWrapperIfData->stopFrame = par.stopFrame;

pFusionWrapperIfData->startFrame = *_currentFrame+1;
pFusionWrapperIfData->fusedFrames = pFusionWrapperIfData->startFrame;

	pFusionWrapperIfData->poses = poses;
	//pFusionWrapperIfData->pLast = poses.back();
pFusionWrapperIfData->lastFrame = std::min(pFusionWrapperIfData->stopFrame, poses->back().size());

    pFusionWrapperIfData->depthNames = depthNames;
	//pFusionWrapperIfData->depthLast = depthNames.back();

	pFusionWrapperIfData->rgbNames = rgbNames;
	//pFusionWrapperIfData->rgbLast = rgbNames.back();

	pFusionWrapperIfData->newMesh = newMesh;
//--------
    return pFusionWrapperIfData;
}


void OnlineFusionViewerInterface::fusionWrapperIf(
...
//-------- begin: refactored into fusionWrapperIfInit()
	//fprintf(stderr,"\nStarting separate Fusion Thread on Frame %li\n",*_currentFrame+1);
//	fprintf(stderr,"\nStarting fusionWrapperIf() on Frame %li\n",*_currentFrame+1);
...
//    printf("@@@ orig depthLast.size(): %ld\n", depthLast.size());
//	printf("@@@ orig rgbLast.size(): %ld\n", rgbLast.size());
//	printf("@@@ orig pLast.size(): %ld\n", pLast.size());
//	return; //for debug
//-------- end: refactored into fusionWrapperIfInit()
//    FusionWrapperIfData *pFusionWrapperIfData = fusionWrapperIfInit(
//        &depthNames, &rgbNames, &poses, par, _currentFrame, _currentTrajectory,
//        newMesh, fusionActive, fusionAlive);
//-------- end: refactored into arg


    //should be all 2486
    std::vector<std::string> &depthLast = pFusionWrapperIfData->depthNames->back();
    std::vector<std::string> &rgbLast = pFusionWrapperIfData->rgbNames->back();
    std::vector<CameraInfo> &pLast = pFusionWrapperIfData->poses->back();
    // for the signature of FusionWrapperIfOneStep()
    std::vector<std::string> *p_depthLast = &pFusionWrapperIfData->depthNames->back();
    std::vector<std::string> *p_rgbLast = &pFusionWrapperIfData->rgbNames->back();
    std::vector<CameraInfo> *p_pLast = &pFusionWrapperIfData->poses->back();


	printf("@@@ depthLast.size(): %ld\n", depthLast.size());
	printf("@@@ rgbLast.size(): %ld\n", rgbLast.size());
	printf("@@@ pLast.size(): %ld\n", pLast.size());
	if(depthLast.size() != rgbLast.size()){
		fprintf(stderr,"\nERROR: The last Depth and RGB Name Vectors have different Size!");
		return;
	}
	printf("[ok] par.imageDepthScale: %f\n", pFusionWrapperIfData->imageDepthScale);

	volatile cv::Mat **depthImageBuffer = new volatile cv::Mat*[depthLast.size()];
#ifdef PREPROCESS_IMAGES
	volatile std::vector<cv::Mat> **rgbSplitImageBuffer = new volatile std::vector<cv::Mat> *[pFusionWrapperIfData->rgbLast.size()];
#else
    ...
    if (pFusionWrapperIfData->threadImageReading) {
  	    fprintf(stderr,"\nStarting Image Reading in decoupled Thread");
#ifdef PREPROCESS_IMAGES
  	    imageThread = new boost::thread(imageReadingAndPreprocessingWrapperIf, depthLast, pFusionWrapperIfData->rgbLast, depthLast.size(),
  			startFrame, depthLast.size(),depthImageBuffer,rgbSplitImageBuffer,&readingActive,maxCamDistance);
#else
  	    imageThread = new boost::thread(imageReadingWrapperIf, depthLast, rgbLast,
  			pFusionWrapperIfData->startFrame, pFusionWrapperIfData->lastFrame, depthImageBuffer, rgbImageBuffer, &readingActive, pFusionWrapperIfData->maxCamDistance, &pFusionWrapperIfData->fusedFrames);
#endif
    }

    fprintf(stderr, "\n@@@ %s(): poses.size(): %ld\n", __FUNCTION__, pFusionWrapperIfData->poses->size());
	if(pFusionWrapperIfData->poses->size()<=1){

		//TODO: Warum funzt das nicht auch ohne volatile size_t und nur mit volatile pointern?!
//		for(volatile size_t currentFrame = pFusionWrapperIfData->startFrame; *fusionAlive && currentFrame < pFusionWrapperIfData->lastFrame;){
		for(volatile size_t currentFrame = pFusionWrapperIfData->startFrame; *fusionAlive && currentFrame < pFusionWrapperIfData->lastFrame;){
			if(*fusionActive){
				if(pFusionWrapperIfData->threadImageReading){
...
//					fusedFrames = currentFrame;
				...
				//-------- begin: refactored into fusionWrapperIfOneStep()
//					fprintf(stderr,"\n+++ %s(): Add Depthmap %li <<<<<<<< <<<<<<<< <<<<<<<<\n",__FUNCTION__,currentFrame);
//					cv::Mat depthimage = cv::imread(depthLast[currentFrame],-1);
//					cv::Mat rgbimage = cv::imread(rgbLast[currentFrame]);
//#ifdef PREPROCESS_IMAGES
...
//                    //sleep(999999);  // 1.1gb RAM consumed at this point
//                    //fusion->writePLY("mesh-first__renameme");  exit(0);
                //-------- end: refactored into fusionWrapperIfOneStep()
                    fusionWrapperIfOneStep(currentFrame,
                        p_depthLast,
                        p_rgbLast,
                        pFusionWrapperIfData->fusion,
                        p_pLast,
                        pFusionWrapperIfData->imageDepthScale,
                        pFusionWrapperIfData->maxCamDistance,
                        pFusionWrapperIfData->newMesh);
					currentFrame++;
				}
...
void OnlineFusionViewerInterface::updateSlot()
{
	if(_threadFusion){
		if(_poses.size()==1) {
		...
		if(!_fusionThread){
		    fprintf(stderr, "\n@@@ %s(): spawning the fusion thread!!!", __FUNCTION__);
// orig version:
//			_fusionThread = new boost::thread(fusionWrapperIf,_depthNames,_rgbNames,_poses,
...
//					&_currentFrame,&_currentTrajectory,&_newMesh,&_fusionActive,&_fusionAlive);
            ...
            FusionWrapperIfData *pFusionWrapperIfData = fusionWrapperIfInit(&_depthNames,&_rgbNames,&_poses,
					FusionParameter(_fusion,_imageDepthScale,_maxCamDistance,_threadImageReading,_nextStopFrame),
					&_currentFrame,&_currentTrajectory,&_newMesh,&_fusionActive,&_fusionAlive);
			_fusionThread = new boost::thread(fusionWrapperIf,
        ...
        fprintf(stderr, "\n@@@ %s(): _newMesh: %d _createMeshList: %d\n", __FUNCTION__, _newMesh, _createMeshList);
		if(_newMesh){
			_newMesh = false;
			...
			if(_createMeshList){
                ...
                fprintf(stderr, "\n%s(): 11Setting _meshCurrent to _currentMeshInterleaved with _fusion->getMeshInterleavedMarchingCubes()",__FUNCTION__);
...
#endif
#endif
				fprintf(stderr,"\n@@@ %s(): Copied Mesh, creating Pointer Mesh",__FUNCTION__);
//				_pointermeshes[0] = new PointerMeshDraw(*_currentMeshForSave,_lightingEnabled? 1 : 0);
				fprintf(stderr,"\n@@@ %s(): Pointer Mesh created",__FUNCTION__);
			}
		}
	}
	else{
...
#ifndef DEBUG_NO_MESHES
#ifndef DEBUG_NO_MESH_VISUALIZATION
                            fprintf(stderr, "\n%s(): 00Setting _meshCurrent to _currentMeshInterleaved with _fusion->getMeshInterleavedMarchingCubes()",__FUNCTION__);
							*_currentMeshInterleaved = _fusion->getMeshInterleavedMarchingCubes();
							if(_lightingEnabled) _pointermeshes[0] = new PointerMeshDraw(*_currentMeshInterleaved,_lightingEnabled? 1 : 0);
#endif
#endif
